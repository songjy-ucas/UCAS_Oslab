...
#define CYCLE_PER_MOVE 10 // 定义了飞机每移动一格，内层循环需要迭代多少次,控制动画的速度。值越大，内层循环执行次数越多，飞机移动越慢。
#define LENGTH 60 // 飞机移动总长度
#define CHECK_POINT 10 // 检查点位置
	...
	printf("%c", '|');
        ...
        for (int i = (60 - LENGTH) * CYCLE_PER_MOVE; i < 60 * CYCLE_PER_MOVE; i++)
        {
            ...
            if(i % CYCLE_PER_MOVE == 0) // 只有当 i 是 CYCLE_PER_MOVE 的整数倍时，才执行一次绘制操作。实现了循环 CYCLE_PER_MOVE 次，飞机移动一格的效果。
            {
                ...
                printf("[fly1] position: %d path.%d phase",current_pos,phase);
            }
        }
        ...
        printf("[fly1] used time per round: %d tick.%d round",clk,round);
    }
}